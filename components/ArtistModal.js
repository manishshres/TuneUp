  ...
  return (
    ...
    <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center z-50">
      ...
      {url ? (
        ...
        <div key={url.id} className="bg-white rounded-lg max-w-2xl p-8 z-50 mb-2 overflow-y-auto">
          ...
          <div>
            <h2 className="text-left text-lg font-bold mt-2 mb-2"  >Popular Releases: </h2>
            <div className="grid grid-cols-2 gap-4 mt-2">
              
{/* Display the top 4 album from artist */}
 {url?.discography?.popularReleases?.items.slice(0, 10).map((item, i) => (
              <div className="border border-gray-300 p-4 rounded-md flex flex-col md:flex-row">
                  <div className="md:w-1/3 mb-4 md:mb-0 md:pr-4">
                    <img src={item?.releases?.items[0]?.coverArt?.sources[1].url} className="object-cover object-center w-full h-auto rounded-md" />
                  ...
                  <div className="md:w-2/3">
                    <h2 className="font-bold text-md">{item?.releases?.items[0].name}</h2>
                    
                    <p>Released: {item?.releases?.items[0].date?.year} <br />Total Tracks: {item?.releases?.items[0].tracks?.totalCount} </p>               
                  </div>
                </div>
